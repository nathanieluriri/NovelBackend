...
// Rendered as the Scalar "Introduction" (Markdown). Backticks are kept out of a
// template literal by joining plain strings.
const INFO_DESCRIPTION = [
  "Next.js port of the v2 API.",
  "",
  "## Response format",
  "Every 2xx JSON response is wrapped in a success envelope:",
  "",
  '```json',
  ...
  "",
  '```json',
  '{ "success": false, "message": "<reason>", "data": null, "errors": [ ... ] }',
  '```',
  "",
  "List endpoints put a `PaginatedListOut` in `data`: `{ items, meta, summary }`.",
  "",
  "## Authentication",
  "Most endpoints require a **Bearer JWT**, sent as a header:",
  "",
  '```',
  "Authorization: Bearer <accessToken>",
  '```',
  "",
  "Auth is **stateful**: a valid signature is not enough — the token's inner id",
  "must resolve to a live server-side session (deleted on logout / after ~10 days),",
  "so tokens can be revoked server-side.",
  "",
  "### 1. Get a token",
  "- `POST /api/v2/user/sign-in` (credentials) → `data.accessToken` + `data.refreshToken`",
  "- `POST /api/v2/user/sign-up` to create a credentials user (also returns tokens)",
  "- Google sign-in: open `GET /api/v2/user/google/auth` in a browser, then",
  "  `POST /api/v2/user/google/exchange` with the returned one-time `code`",
  "- Admins: `POST /api/v2/admin/sign-in`, then activate the token with the emailed",
  "  OTP via `POST /api/v2/admin/verify` before it can be used",
  "",
  "### 2. Use it on this page",
  "Click **Authorize** (top-right), paste the `accessToken`, and every **Try it out**",
  "call will include the `Authorization` header automatically.",
  "",
  "### 3. Expiry & refresh",
  "Access tokens expire after ~15 minutes. To rotate, send the (expired) access token",
  "in the `Authorization` header **and** the refresh token in the body to",
  "`POST /api/v2/user/refresh` (members) or `POST /api/v2/admin/refresh` (admins).",
  "You receive a brand-new access + refresh pair; the old ones are revoked.",
  "",
  "### Roles",
  "- **Member** token — regular reader endpoints.",
  "- **Admin** token — admin endpoints; must be OTP-activated.",
  "- Endpoints shown without a lock are public (no token required).",
  "",
  "### Example",
  '```bash',
  "TOKEN=$(curl -s -X POST https://<host>/api/v2/user/sign-in \\",
  "  -H 'content-type: application/json' \\",
  '  -d \'{"email":"you@example.com","password":"..."}\' | jq -r .data.accessToken)',
  "",
  'curl https://<host>/api/v2/user/details -H "Authorization: Bearer $TOKEN"',
  '```',
  "",
  "## Pagination",
  "List endpoints accept `skip` (default 0) and `limit` (default 20, max 100) and",
  "return `meta: { skip, limit, returned, total, hasMore }` alongside the items.",
].join("\n");
