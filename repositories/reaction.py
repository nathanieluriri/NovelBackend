...
async def ensure_reaction_indexes() -> None:
    ...
    async with _reaction_indexes_lock:
        ...
        await db.reactions.create_index(
            [("userId", ASCENDING), ("authorRoomId", ASCENDING)],
            unique=True,
            background=True,
        )
...
async def get_reaction_by_user_and_room(user_id: str, author_room_id: str) -> Optional[ReactionOut]:
...
async def get_reactions_by_user_for_author_room_ids(
    ...
    cursor = db.reactions.find(
        {
            "userId": user_id,
            "authorRoomId": {"$in": author_room_ids},
        }
    )
